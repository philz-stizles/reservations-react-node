...
export const activateAccount = (req: Request, res: Response) => {
  ...
  jwt.verify(
    ...
    process.env.JWT_ACCOUNT_ACTIVATION as string,
    // eslint-disable-next-line consistent-return
    async (jwtError: any, decodedToken: any) => {
      if (jwtError) {
...
const login = async (req: Request, res: Response, next: NextFunction) => {
  ...
  // Check if user exists
  const existingUser = await User.findOne({ where: { email } }); // There is difference between adding
...
const forgotPassword = async (
  ...
  await existingUser.save(); // At this point you are setting password reset token and saving
